...
Having a small part of LiFePO4 BESS capacity reserved and its total power available in a hybrid PV system, the IPLPF smoothing costs only a 5% increase in the BESS capacity. Such a possibility is significant in grid areas with a high concentration of small-scale hybrid PV systems.  
EDLC supercapacitors offer cost-effective IPLPF smoothing for the utility-scale PV plants. Even the price of EDLC is expected to drop to 1000 €/kWh in the future.

### Summary of IPLPF
Long-term continous GI(t) measurement has confirmed that if the smoothing performed close ideal, it would be affordable either with LiFePO4 or EDLC storage technology properly implemented in PV systems. The wind power exhibits a similar intermittency like the PV power, but with a significant exception: A relatively weak influence of daily cycles on wind causes that the low spectral component of wind power is random. Therefore Δt=τ~g~(0) is not always the optimal lead time in order to minimize the requested ESS capacity.  
