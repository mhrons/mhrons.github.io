...
The graphs on the left show the measured signal GI(t), low-passed GI~s~(t) (legend “lp”), and left-shifted low-passed GI~s~(t+Δt) (legend "aavg0") on a day with high solar exposure and strong intermittency. The graphs on the right show the specific energy GX(t) [Wh/m^2^] accumulated by IPLPF (legend "aavg0") vs GX(t) accumulated by LPF (legend "lp") on the same day. Exactly predicted input signal GI(t+Δt) minimizes the standard deviation of GX(t), in practise minimizing the difference ΔGX = max(GX) – min(GX), thus defining the requirement for a specific accumulation capacity. The specific throughput of the accumulated energy is also minimized by the IPLPF smoothing.  
