...
In addition to the PLPF, we have developed a “smart predictive" low-pass filtering method [SPLPF](https://mhrons.github.io/splpf/), minimizing the accumulated energy \eqref{eq:4}, \eqref{eq:5} with respect to the real-predicted values GI~f~ and given ramping limit. We eventually analyzed the performance of the following smoothing methods:
