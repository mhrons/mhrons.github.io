...
OLAP cube is a multi-dimensional set of pre-aggregated accumulation rates along partial dimensions (a few of independent quantitative or categorical variables), provided a constant smoothing effect meets the given ramping limit of the filtered power. Let us define the reference smoothing as the output of a 3rd-order Butterworth filter with the cut-off frequency = 7.5/12h (Δt = 30 minutes). Our OLAP cube consists of the following dimensions:
