...
In addition to PLPF, we have developed a “smart predictive" low-pass smoothing method ***SPLPF*** (patent pending), ***minimizing the accumulated energy \eqref{eq:4}, \eqref{eq:5} with respect to the given ramping limit and real-predicted values GI~f~***. Eventually, we analyzed the performance of the following smoothing methods:
