...
defines the change of its state-of-charge SOC [Wh] since time=0 until time=t, while SOC may acquire both positive and negative values. The time advance Δt reduces absolute values of the integrated function. Always-positive SOC values can be technically managed in BESS. The above-defined time advance Δt ensures that following technical criteria are satisfied:  
...
The value delta(SOC) should be predicted and BESS must be charged to delta(SOC)/2 at the begining of each cycle T (24 hours in case of PV energy) in order to always keep SOC≥0.  
...
The rate of accumulation is expressed by (4) and (5) which are minimized by Δt during a working cycle T. The rate of accumulation is also expressed by the extreme BESS power demand p(τ)-s(τ+Δt) during a sudden loss or peak of direct sunlight. This value results from the installed power of PVPP.
...
The measured signal GI(t) determines the specific accumulation parameters maximum specific power max|GI(τ)-GIs(τ+Δt)| [W/m2] and maximum specific energy delta(GX)=max(GX)-min(GX) [Wh/m2] required for the smoothing by IPLPF method. Although IPLPF shrinks the necessary delta(GX) hence delta(SOC) and the total accumulated throughput to their theoretical minimum, it reciprocally increases the relative smoothing power (Pmax x 1h)/delta(SOC) up to 10C, since no algorithm can supply the missing PV power while the direct Sun beams are temporarily obscured by clouds. The lithium accumulators are cheap, efficient and fast, but not enough powerful for a sole IPLPF smoothing. It is not recommended to charge/draw a Li-Ion battery with relative power >> 1, otherwise the battery’s life cycle would be curtailed. Such a high relative power is provided by EDLC supercapacitors (SC), but on contrary to Lithium bateries, these do not provide enough cheap capacity to satisfy the IPLPF smoothing. The currently-available accumulation technology of electric energy is not harmonized with demands of the sole IPLPF smoothing.  
...
: 1) Its max(SOC) poses no limit to max(P) when applied for IPLPF smoothing. 2) IPLPF accumulates energy only on solar-intermittent days (else no accumulation is needed), and the flywheels only wear out when they are rotating - mainly those with mechanical bearings. Because of unknown prices and life cycle parameters, we calculated neither CapExp, nor OpExp of the flywheel in a role of IPLPF accumulator. However, ***the native synergy between demands and features identifies the flywheel as a suitable accumulator for IPLPF smoothing.***

#### Large-scale PV plant
Water pumps provide efficient storage for large scale PV plants (cheaper and cleaner than a corresponding BESS), but no power smoothing because the water pumps are too sluggish for such a task. Large-scale PV intermittency can be smoothed by a relatively low accumulation capacity =delta(SOC) and large relative power up to 10C. 
...
Both errors are applied at the "future" time t+Δt, whereas the LPF group delay hence Δt depend on the selected rate of PV power smoothing defined by the order and cut-off frequency of LPF. In the simulation, we control the prediction error by 2 quantitative (OLAP) dimensions:
