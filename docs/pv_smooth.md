...
[PV predictor](https://www.sciencedirect.com/science/article/abs/pii/S0038092X14001327)
  trained by a sky imagery and by GI(t):**
...
The measured signal GI(t) determines the specific accumulation parameters max(GI) [W/m2] and delta(GX)=max(GX)-min(GX) [Wh/m2] required for the smoothing by IPLPF method. Although IPLPF shrinks the necessary delta(GX) hence delta(SOC) and the total accumulated throughput to their theoretical minimum, by contrast it increases the relative smoothing power (Pmax x 1h)/delta(SOC) up to 10C, since no algorithm can supply the missing PV power while the direct Sun beams are temporarily obscured by clouds. The lithium accumulators are cheap, efficient and fast, but not enough powerful for a sole IPLPF smoothing. It is not recommended to charge/draw a Li-Ion battery with relative power >> 1, otherwise the battery’s life cycle would be curtailed. Such a high relative power is provided by EDLC supercapacitors (SC), but on contrary to Lithium bateries, these do not provide enough cheap capacity to satisfy the IPLPF smoothing. The currently-available electric accumulation technology is not harmonized with demands of the sole IPLPF smoothing.  
...
: 1) Its max(SOC) poses no limit to max(P). 2) IPLPF accumulates energy only when the solar intermittency persists (else no accumulation is needed), and the flywheels only wear out when they are rotating - mainly those with mechanical bearings. 3) Zero kinetic energy of FESS matches with mean(GX)=0 in the IPLPF smoothing, since FESS operates with both positive and negative inertia momentum. (With BESS, the value delta(SOC)/2 must be predicted and offset in advance to match with GX=0.) Because of unknown prices and life cycle parameters, we calculated neither CapExp, nor OpExp of the flywheel in a role of IPLPF accumulator. However, ***the native synergy between demands and features identifies the flywheel as a suitable accumulator for IPLPF smoothing.***
