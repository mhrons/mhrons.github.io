...
[PV predictor](https://www.sciencedirect.com/science/article/abs/pii/S0038092X14001327)
  trained by the sky imagery and GI(t) signals. We further integrate the two into one entity, utilizing the advantage of predictive filters.
...
defines the change of its state-of-charge [Wh] since time=0 until time=t, while SOC may acquire both positive and negative values. (Always-positive SOC values in ESS can be technically managed.) The time advance Δt reduces absolute values of the integrated function. The IPLPF-defined time advance Δt ensures following technical criteria to be satisfied:  
...
Global irradiance GI measured (legend meas), filtered by LPF (legend lp) and by IPLPF (legend aavg0), and corresponding specific accumulated energy GX:  
...
Both errors are applied at the "future" time t+Δt where the advance Δt (LPF group delay) result from the demanded quality of smoothing. We control the group delay by the order and cut-off frequency of LPF.  
In our simulation, we affect the prediction error by 2 quantitative (OLAP) dimensions:
...
The results of analysis have been tabelized and plotted. The impact of prediction error (see Figures 2, 6) exhibits the following empiric dependencies of the tracking criteria by the OLAP partial dimensions:
...
- Filter order does not affect the power smoothing quality by PLPF method. For small filter orders, the smoothing by PLPF is better than that of SPLPF.
...
- Increasing the filter order from 1 to 3 notably reduces the accumulation rate induced by both IPLPF and SPLPF smoothing methods, while the SPLPF smooths with a non-zero prediction error (Figures 13-15, 19-21). ***With the simulated predictor, SPLPF performs best with the filter order 3.***
