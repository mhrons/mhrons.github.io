...
Although the IPLPF shrinks the necessary capacity and the accumulated throughput to their theoretical minimum, it reciprocally increases the relative smoothing power GI~ESS~/ΔGX, as it is shown in [tables 2 and 3](https://mhrons.github.io/pv_smooth/#accumulation-rate-by-method). Regardless of its small capacity, the ESS must supply the missing PV power while the direct sun beams are temporarily shadowed by clouds, or vice versa.  
If the ESS does not otherwise provide [synthetic inertia](https://mhrons.github.io/pv_intermit/#smoothing-synthetic-inertia) to the grid, its capacity could be further minimized (probably ΔGX < 40 Wh/m^2^): Knowing the infeed tariff, price and the service interval of ESS, ΔGX could be optimized by equalizing the margins of the lost PV energy and saved ESS capacity. However, accounting for damage caused by the raw PV infeed in 3) is more complex.
