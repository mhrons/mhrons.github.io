...
* Using the measured time series GI(t), we can "ex-post" excite the low-pass filter with a "predicted" input signal GIf(t+Δt). That is, a shifted measured signal artificially biased by the simulated prediction error simulates the operation of a real predictor. Given the predicted signal and the desired quality of smoothed PV power, the accumulated energy by the filter should be minimized, hence minimized costs of the PV power smoothing.
* However low is the accumulated energy by smoothing, the PV smoothing is not for free. It pays-off only if a favourable "smooth feed-in tariff" exists. Ramping limits of the smooth PV infeed should be legalized by a technical standard. Once the the ramping limits are given and the prediction error is known and the filter model is optimized, the accumulation rate and corresponding costs can be aggregated from the measured GI signal. Hence a trade-off between the smoothing costs and a corresponding reduction in the grid control costs can be balanced. This is, how the PV ramping limit and the corresponding feed-in tariff can be optimized.
