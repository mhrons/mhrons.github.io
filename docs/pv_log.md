...
* In addition to the measurement, the PV sensors supply power to the data logger and charge its backup battery. The off-grid measurement system is self-powered 24 hours x 7 days a week. The system requires a minimum maintenance.
* Simultaneous measurement of GI from 4 PV panels with size 20 cm x 16 cm mounted in different angles allows an accurate interpolation of GI at any plane of incidence assuming that the incidence vector points among the 4 panels' normal vectors (intra-normal area). GI(t) signal from the south-oriented panel is supplied to the numeric simulation of PV power smoothing. Its normal vector is elevated by 47°. (This panel is refered to as PV2.)
